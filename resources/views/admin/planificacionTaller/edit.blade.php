    ...
    <div>
        <div class="w-full">
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg px-8 pb-3">
                <h1 class="uppercase text-center mt-4 text-2xl">Editar la Planificación</h1>
                <form action="{{ route('admin.planificacionTaller.update', $planificacion->id) }}" method="post">
                    @csrf
                    @method('PUT')
                    <div class="mt-4">
                        <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                            <div>
                                ...
                                <select class="custom-select sm:text-sm" id="taller" name="taller">
                                    <option value="" selected disabled>-- Seleccionar taller --</option>
                                    @foreach ($talleres as $taller)
                                        <option
                                            value="{{ $taller->id }}"
                                            {{$planificacion->taller_id == $taller->id ? 'selected' : ''}}
                                        >{{ Str::title($taller->nombre) }}</option>
                                    @endforeach
                                </select>
                            ...
                            <div>
                                ...
                                <select class="custom-select sm:text-sm"
                                    ...
                                    @foreach ($docentes as $docente)
                                        <option
                                            value="{{ $docente->id }}"
                                            {{$planificacion->docente_id == $docente->id ? 'selected' : ''}}
                            ...
                            <div>
                                ...
                                <select class="custom-select sm:text-sm" id="horario" name="horario" value="{{ old('horario') }}">
                                    ...
                                    @foreach ($horarios as $horario)
                                        <option
                                            value="{{ $horario->id }}"
                                            {{$planificacion->horario_id == $horario->id ? 'selected' : ''}}
                                            >{{ $horario->horario_completo }}</option>
                                    @endforeach
                                </select>
                            ...
                            <div>
                                ...
                                <select class="custom-select sm:text-sm" id="modalidad" name="modalidad" value="{{ old('modalidad') }}">
                                    ...
                                    @foreach ($modalidades as $modalidad)
                                        <option
                                            value="{{ $modalidad->id }}"
                                            {{$planificacion->modalidad_id == $modalidad->id ? 'selected' : ''}}
                        ...
                        <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                            <div>
                                ...
                                <x-jet-input id="fecha_inicio" type="date" name="fecha_inicio"
                                    class="mt-1 block w-full" value="{{ $planificacion->fecha_inicio->format('Y-m-d') }}"/>
                                <x-jet-input-error for="fecha_inicio" class="mt-2" />
                            </div>
                            <div>
                                <x-required-label for="fecha_fin" value="Fecha de Finalización" />
                                <x-jet-input id="fecha_fin" type="date" name="fecha_fin"
                                    class="mt-1 block w-full" value="{{ $planificacion->fecha_fin->format('Y-m-d') }}"/>
                    ...
                    <div class="mt-4">
                        <div class="grid grid-cols-1 sm:grid-cols-3 gap-4">
                            <div>
                                ...
                                <x-jet-input id="duracion" type="number" name="duracion"
                                    class="mt-1 block w-full" value="{{ $planificacion->duracion }}"/>
                                <x-jet-input-error for="duracion" class="mt-2" />
                            </div>
                            <div>
                                <x-required-label for="costo" value="Costo del Taller" />
                                <x-jet-input id="costo" type="number" name="costo"
                                    class="mt-1 block w-full" value="{{ $planificacion->costo }}"/>
                                <x-jet-input-error for="costo" class="mt-2" />
                            </div>
                            <div>
                                <x-required-label for="carga_horaria" value="Carga Horaria" />
                                <x-jet-input id="carga_horaria" type="number" name="carga_horaria"
                                    class="mt-1 block w-full" value="{{ $planificacion->carga_horaria }}"/>
                    ...
                    <div class="mt-4">
                        ...
                        <x-jet-input id="requisitos" type="text" name="requisitos"
                            class="mt-1 block w-full" value="{{ $planificacion->requisitos }}"/>
