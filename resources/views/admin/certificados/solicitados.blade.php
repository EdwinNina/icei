    ...
    <div>
        <div class="w-full">
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg p-6">
                <h1 class="text-gray-500 uppercase text-2xl mt-1 mb-3 text-center">Solicitud de Certificados de Estudiantes</h1>
                                        ...
                                        <tr>
                                            ...
                                            <td class="w-44">
                                                <div class="text-sm text-gray-900">
                                                    {{ Str::title($item->inscripcion->planificacionCarrera->carrera->titulo)}}
                                                </div>
                                                <div class="text-xs font-bold text-gray-900"
                                                data-toggle="tooltip" data-placement="left" title="{{$item->inscripcion->modulo->titulo_completo}}">
                                                    {{ Str::substr($item->inscripcion->modulo->titulo_completo, 0, 30) . '...'}}
                                                </div>
                                            </td>
                                            <td class="table-tail-td text-center">
                                                <div class="text-sm text-gray-900">{{$item->nota->nota_final}}</div>
                                            </td>
                                            <td class="table-tail-td text-center">
                                                <div class="text-sm text-gray-900">
                                            ...
                                            <td class="table-tail-td text-center text-sm font-medium">
                                                <a href="{{ route('admin.certificados.generarCertificado', $item->id) }}"
                                                    target="_blank"
...
@section('js')
    ...
    <script>
        function activarImpresion(e, id){
            if(e.target.checked){
                axios.post("/admin/certificados/activarImpresion", {id : id})
                .then( resp => {
                    if(resp.status === 200){
                ...
                axios.post("/admin/certificados/desactivarImpresion", {id : id})
                .then( resp => {
                    if(resp.status === 200){
