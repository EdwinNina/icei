...
@section('content')
    ...
    @php
        ...
        $rol_actual = Auth::user()->getRoleNames();
    @endphp
        ...
        <div class="shadow-sm overflow-hidden border-b rounded border-gray-200 sm:rounded-lg my-4">
            <form action="{{ route('admin.notas.inscritos.store') }}" method="post">
                ...
                @if ($usuario === null)
                                        ...
                                        <td class="text-center">
                                            ...
                                            <div class="text-xs">
                                                <span class="text-gray-700">
                                                   Nota subida el <strong>{{$item->updated_at->translatedFormat('d F Y')}}</strong>
                                                </span>
                                            </div>
                                        </td>
                                    </tr>
                    ...
                    <div class="mt-3 mx-4">
                        <span class="uppercase font-bold">Observaciones del Módulo</span>
                        <p class="text-sm mt-2 text-gray-600">
                            {{$planificacionModulo->detalleNota ? $planificacionModulo->detalleNota->observaciones : 'Sin observaciones aún'}}
                        </p>
                    </div>
                ...
                @else
                    ...
                    <div class="mt-3 mx-4">
                        <span class="uppercase font-bold">Observaciones del Módulo</span>
                        <textarea
                            class="w-full text-sm border-gray-300 focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50 rounded-md shadow-sm mt-1 mr-2 py-2"
                            name="observaciones">{{$planificacionModulo->detalleNota ? $planificacionModulo->detalleNota->observaciones : ''}}</textarea>
                    </div>
                    <div class="my-4 px-6 flex justify-end items-center">
                        <x-back-button href="{{ route('docente.notas.index')}}">Volver</x-back-button>
                        @if ($usuario->generable_id === $docente_id && $habilitar)
