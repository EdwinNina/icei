<div class="my-4" x-data="{mostrarCarritoInscripcion: false}">
    <h2 class="text-sm uppercase text-blue-500 border-b-2 border-gray-200">Detalle Pagos de Inscripción del Taller</h2>
    <div  class="table-responsive bg-gray-50 pb-3 shadow-md rounded-md mt-6">
        <table class="table-tail" id="tabla-pago">
            ...
            <tbody>
                ...
                @foreach ($inscripcion->pagosInscripcionTaller as $index => $pago)
                    @if ($pago->tipoDeRazon->nombre == "taller" && $pago->estado != 0)
    ...
    @if ($inscripcion->saldo > "0")
        ...
        <form action="{{ route('admin.inscripcionesTalleres.update', $inscripcion->id) }}" method="POST">
            @csrf
            @method('PUT')
